 ...
 */
export default function parseLyric(string) {
  const lyricPattern = /\[(\d{2}):(\d{2})\.(\d{2,3})\]/,
    lyric = string.split('\n').map(string => {
      const lyricPatternResult = lyricPattern.exec(string)

      if (!lyricPatternResult) return

      const minute = lyricPatternResult[1] * 60 * 1000,
        second = lyricPatternResult[2] * 1000,
        Millisecond = lyricPatternResult[3].length === 2 ? lyricPatternResult[3] * 10 : lyricPatternResult[3] * 1,
        time = minute + second + Millisecond,
        text = string.replace(lyricPattern, '')
